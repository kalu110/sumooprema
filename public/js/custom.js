	...
	for(let i of product){
		if(el.value.includes('/')){
			...
			if(i.name.trim().toLowerCase() == a[0].trim().toLowerCase() && i.category.trim().toLowerCase() == a[1].trim().toLowerCase()){
				...
				<th scope="row">${br}</th>
				<td style="width:150px;"><img src="files/${photo[1]}" style="width:60%;height:60px;" alt=""></td>
				<td>${i.name}</td>
				<td>${i.category}</td>
			...
			if(brnamKategorija != 0){
				...
				for(let j of product){
					...
					if(i.name == j.category && brnam == j.brend){
						...
						<!-- Single Product -->
						<div class="col-8 offset-2 offset-sm-0 col-sm-6 col-md-4 col-lg-3">
